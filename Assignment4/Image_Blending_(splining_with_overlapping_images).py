...
import numpy as np
import sys
from Image_Pyramids import laplacian_pyramid,gaussian_pyramid,reconstructed
...
# gp1 = gaussian_pyramid(layer1,levels=4)
# gp2 = gaussian_pyramid(layer2,levels=4)
...
# Laplacian pyramid
lp1,gp1 = laplacian_pyramid(layer1,levels=5)
lp2,gp2 = laplacian_pyramid(layer2,levels=5)
# for i in range(1, 5):
...
for la,lb in zip(lp1, lp2):
	...
	cv2.imshow("mix_laplacian"+str(n),ls)
	cv2.waitKey(0)
	n+=1
	LS.append(ls)
...
corrected_image = reconstructed(LS)
# reconstructed image
cv2.imshow("corrected", corrected_image)
